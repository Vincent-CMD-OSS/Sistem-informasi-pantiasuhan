...
<form method="POST" action="{{ route('jadwal.operasional.update') }}">
    @csrf
    <table border="1">
        <tr>
            <th>Hari</th>
...
<h2>Jadwal Khusus</h2>
<form method="POST" action="{{ route('jadwal.khusus.store') }}">
    @csrf
...
@foreach ($jadwalKhusus as $khusus)
    <li>
        ...
        - {{ $khusus->keterangan }}
        <form action="{{ route('jadwal.khusus.destroy', $khusus->id) }}" method="POST" style="display:inline;">
            @csrf @method('DELETE')
